...
## Data Files
The repository provides country-level data, scenario-specific parameters, and output data used in the analysis. The contents include:
...
-	Sensitivity analysis profiles: Input datasets for the workload and grid carbon-intensity sensitivity analyses, including representative training and inference workload profiles (Training_profile.xlsx and Inference_profile.xlsx) and country-specific grid carbon-intensity profiles representing seasonal variation during 2025 (CI_2025_Profile.xlsx).
-	Output data: Optimization results for the intervention pathways, including baseline scenario outputs (int_res_intervention-pathway_climate-pathway.xlsx), the corresponding workload sensitivity cases for the SSP2-45 pathway (int_res_intervention-pathway_SSP2-45_Training.xlsx and int_res_intervention-pathway_SSP2-45_Inference.xlsx), expanded technology portfolio evaluations incorporating SMRs, hydrogen-based energy storage, and biomass-based electricity generation (int_res_expanded_portfolios.xlsx), and off-site carbon-removal evaluations considering BECCS, DAC, ocean-based carbon capture, enhanced rock weathering, and biomass storage (int_res_offsite.xlsx).
## Code files
